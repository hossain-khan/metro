 ...
 * ```
 *
 * Metro will generate a wrapper class with the same name as the function (capitalized as a class
 * name) that you can access from a dependency graph and invoke.
 *
 * ```
 * @DependencyGraph
 * interface AppGraph {
 ...
 * class App @Inject constructor(private val settings: Settings) {
 ...
 * ```
 *
 * In this case, the generated `App` will also be annotated with `@Stable` for stability.
 *
 * ### Why opt-in?
 *
